...
**현재 시세 정보**
- 현재가: ${((_b = financialData.currentPrice) === null || _b === void 0 ? void 0 : _b.toLocaleString()) || 'N/A'}원
...
**DCF 밸류에이션**
- 적정주가: ${((_c = dcfData.fairValue) === null || _c === void 0 ? void 0 : _c.toLocaleString()) || 'N/A'}원
...
**주요 지표**
- 현재가: ${((_d = data.currentPrice) === null || _d === void 0 ? void 0 : _d.toLocaleString()) || 'N/A'}원
- PER: ${data.per || 'N/A'}
- PBR: ${data.pbr || 'N/A'}
- EPS: ${data.eps || 'N/A'}원
- ROE: ${data.roe || 'N/A'}%
...
${data.MACD && data.MACD < 0 ? '- MACD 하락 → 매도 신호' : ''}`
                        ...
                        case 'calculate_dcf': {
                            const data = await stockData.calculateDCF(args.ticker, args.growth_rate, args.discount_rate);
                            result = {
                                content: [
                                    {
                                        type: 'text',
...
**분석 결과**
- 적정주가: ${((_e = data.fairValue) === null || _e === void 0 ? void 0 : _e.toLocaleString()) || 'N/A'}원
- 현재가: ${((_f = data.currentPrice) === null || _f === void 0 ? void 0 : _f.toLocaleString()) || 'N/A'}원
...
${news.slice(0, 10).map((item, i) => `**${i + 1}. ${item.title}**
- 일시: ${item.date}
- 요약: ${item.summary}
- 감성: ${item.sentiment}
`).join('\n')}`
...
**순매수 금액**
- 외국인: ${((_g = data.foreign) === null || _g === void 0 ? void 0 : _g.toLocaleString()) || 'N/A'}억원
- 기관: ${((_h = data.institution) === null || _h === void 0 ? void 0 : _h.toLocaleString()) || 'N/A'}억원
- 개인: ${((_j = data.individual) === null || _j === void 0 ? void 0 : _j.toLocaleString()) || 'N/A'}억원
...
- 현재가: ${((_a = company.currentPrice) === null || _a === void 0 ? void 0 : _a.toLocaleString()) || 'N/A'}원
- 시가총액: ${((_b = company.marketCap) === null || _b === void 0 ? void 0 : _b.toLocaleString()) || 'N/A'}억원
- PER: ${company.per || 'N/A'}
- PBR: ${company.pbr || 'N/A'}
- ROE: ${company.roe || 'N/A'}%
...
${data.length > 1 && data[0] && data[0].per && data[0].per !== 'N/A' ?
                                                `- ${args.ticker}의 PER이 업계 평균 대비 ${parseFloat(data[0].per) < data.slice(1).reduce((acc, cur) => acc + (parseFloat(cur.per) || 0), 0) / (data.length - 1) ?
                                                    '낮음 (저평가)' : '높음 (고평가)'}` : '- PER 비교 데이터 부족'}`
                                        }
                                    ]
                                };
                            }
            ...
            return res.status(500).json({
                jsonrpc: '2.0',
                id: ((_k = req.body) === null || _k === void 0 ? void 0 : _k.id) || 1,
                error: {
                    code: -32603,
